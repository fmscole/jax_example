...
if __name__ =="__main__":
    ...
    losss=alpha(logits, labels,target_len)
    ...
    optax.ctc_loss(logits=logits,logit_paddings=logit_paddings,labels=targets,label_paddings=label_paddings)

    start=time.time()
    for i in range(10):
        l=optax.ctc_loss(logits=logits,logit_paddings=logit_paddings,labels=targets,label_paddings=label_paddings)
        print(l[0],end=" ")
    print("")
    print(time.time()-start)
